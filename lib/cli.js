...
var schema = {
	properties: {
		...
		pkg: {
			description: 'Create package.json file for each plugins (yes|no) ? [yes]',
			validator: /y[es]*|n[o]?/,
		...
		force: {
			description: 'Force the creation of files even if they exists (yes|no) ? [no]',
			validator: /y[es]*|n[o]?/,
			warning: 'Must respond yes or no',
			default: 'no',
			required: true
...
prompt.get(schema, function (err, cli) {
	...
	async.eachSync(plugins, function(plugin, callback) {
		...
		async.waterfall([
			function createPluginDir(cb) {
				mkdirp(pluginDir, function(err) {
					if(err) {
						utils.promptToContinue('Cannot create plugin directory "'+ pluginDir +'".', err, prompt, cb);
					} else {
						cb();
					}
				});
