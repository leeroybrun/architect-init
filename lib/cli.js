...
prompt.get(schema, function (err, prompted) {
	...
	if(prompted.structure == 'sub') {
		for(var i = 0; i < plugins.length; i++) {
			plugins[i] = path.normalize(plugins[i].replace(new RegExp('.', 'g'), path.sep));
		}
	}
	...
	if(prompted.structure == 'dotted') {
		for(var i = 0; i < plugins.length; i++) {
			plugins[i] = plugins[i].replace(new RegExp(path.sep, 'g'), '.').replace(new RegExp('/', 'g'), '.');
		}
	}
	...
	async.eachSeries(plugins, function(plugin, callback) {
		var pluginName = plugin.replace(new RegExp(path.sep, 'g'), '.').replace(new RegExp('/', 'g'), '.');
